...
export const getIndexReader = (heirarchy, indexNode, getNextInputBytes) => read(
  getNextInputBytes,
...
const newOutputWriter = (flushBoundary, flush) => {
  ...
  return (text) => {
    if (isString(text) && currentBuffer === null) { currentBuffer = Buffer.from(text, 'utf8'); } else if (isString(text)) {
...
const newInputReader = (getNextInputBytes) => {
  ...
  return () => {
    ...
    if (frombytes.length === remainingBytes.length) { return ''; }
